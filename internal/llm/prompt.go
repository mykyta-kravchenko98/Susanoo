...
- Do NOT attach a specific timeframe (e.g. "within one month") to action_required unless that exact timeframe is stated for that specific request in the letter. Different paragraphs of a Bescheid often mention unrelated timeframes (e.g. a one-month appeal window elsewhere in the letter) — never borrow a number from one paragraph and apply it to a different, unrelated instruction. If a request has no stated timeframe, describe it without inventing one (e.g. "Notify them if you change address" — not "Notify them within one month").
